...
def draw_layer(ax, prompt_toks, words, probs, layer, k):
    ...
    for c in range(seq):
        for r in range(k):
            ...
            y = k - 1 - r
            ax.add_patch(
                plt.Rectangle((c, y), 1, 1, facecolor=BLUES(min(prob * 2.0, 1.0)),
            ...
            ax.text(
                c + 0.5, y + 0.5, word[:10], ha="center", va="center",
...
def main():
    ...
    ap.add_argument("--layers", default=None, help="comma-separated, e.g. '14,20,24'; default spreads across depth")
    ...
    fig, axes = plt.subplots(
        len(layers), 1, figsize=(max(7, seq * 1.15), (0.5 * args.k + 0.7) * len(layers) + 1.2),
        squeeze=False,
    )
    ...
    bottom.set_xticklabels(
        [t.strip() or "·" for t in prompt_toks], rotation=40, ha="right", fontsize=10, color=INK
    )
